 ...
 */
Method _cache::parseMethod(const char *data, int linelen)
{
    const char *ptr;
    int	index = 0;
    ptr = (const char *)data;
 ...
 */
char* Cache::isRequest(const char *ptr, const int datalen) {
 ...
 * If it's true, the head end char pointer will be returned, else NULL.
 */
char* Cache::isResponse(const char *ptr, const int datalen) {
	char *eol,*linesp,*head_end = NULL;
        int lnl;
...
Cache::Cache(Analysis* analysis) : Protocol(analysis) {
...
bool Cache::isPacketOf(u_int16_t sport, u_int16_t dport) {
...
bool Cache::isHeaderPacket(const char *ptr, const int datalen) {
...
Status _cache::parseStatus(const char *line, int len) {
...
_protocol::Request* Cache::getRequest(const char *data, const char *dataend, char* time, u_int32_t seq, u_int32_t nxt_seq) {
    return new Request(data, dataend, time, seq, nxt_seq);
}

_protocol::Response* Cache::getResponse(const char *data, const char *dataend, long ack) {
    return new Response(data, dataend, ack);
}

char* Cache::getMethodName(int m) {
    return methodName[m];
}

int Cache::getMethodCount() {
    return static_cast<int>(methodName.size());
}

int Cache::getStatusCount() {
...
_cache::Request::Request(const char *data, const char *dataend, char *time, u_int32_t seq, u_int32_t nxt_seq) {
 ...
 * But only the header fields are extracted.
 */
_cache::Response::Response(const char *data, const char *dataend, long ack) : Response() {
	char *eoh, *eol, *linesp;
